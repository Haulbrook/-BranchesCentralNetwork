...
(function () {
  ...
  window.DEBUG_MODE = new URLSearchParams(window.location.search).has('debug');
      ...
      try {
        ...
        if (result && typeof result.catch === 'function') {
          return result.catch(function (err) {
            if (window.DEBUG_MODE) console.error('[' + (tag || 'safeAsync') + ']', err);
  ...
  window.safeSync = function (fn, tag) {
    return function () {
      try {
        ...
        if (window.DEBUG_MODE) console.error('[' + (tag || 'safeSync') + ']', err);
        showToast((tag ? tag + ': ' : '') + (err.message || 'Unknown error'), 'error');
      }
    };
  };
